...
export const UPDATE_WORD_TO_VOCABULARY = gql`
    mutation UpdateWordToVocabulary(
        $updateWordToVocabulary: UpdateWordToVocabularyInput!
    ) {
        updateWordToVocabulary(updateWordToVocabulary: $updateWordToVocabulary)
    }
`;

const updateWordToVocabulary = async ({
    id,
    userId,
    isFinished,
}: UpdateWordToVocabularyParams) => {
    try {
