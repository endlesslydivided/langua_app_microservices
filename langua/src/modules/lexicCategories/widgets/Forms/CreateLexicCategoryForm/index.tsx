...
const CreateLexicCategoryForm: React.FC<CreateLexicCategoryProps> = ({
    creationCallback,
}) => {
    const { auth } = useAuth();
    ...
    return (
        <form className="flex flex-col gap-5" onSubmit={lcForm.handleSubmit}>
            <TextField
                error={
                    !!lcForm.errors.categoryName &&
                    !!lcForm.touched.categoryName
                }
                helperText={
                    lcForm.errors.categoryName && lcForm.touched.categoryName
                        ? lcForm.errors.categoryName
                        : ''
                }
